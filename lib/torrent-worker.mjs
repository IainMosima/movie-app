  ...
  try {
    ...
    if (req.method === "GET" && path.startsWith("/stream/")) {
      ...
      if (prevVideoFile !== file) {
        if (prevVideoFile) prevVideoFile.deselect();
        file.select();
    ...
    // GET /probe/:infoHash — Return routing decision (useDirectStream) for the video file
    if (req.method === "GET" && path.startsWith("/probe/")) {
      ...
      if (prevProbeFile !== file) {
        if (prevProbeFile) prevProbeFile.deselect();
        file.select();
        selectedVideoFile.set(infoHash, file);
      ...
      // Pessimistic default: if file isn't on disk yet, route to AAC transcode.
      if (!existsSync(diskPath)) {
        console.log(`Probe ${file.name}: file not on disk yet — routing to AAC transcode`);
        return jsonResponse(res, 200, { useDirectStream: false, reason: "file_not_on_disk", durationSec: null });
      }
      ...
      ff.on("close", () => {
        try {
          ...
          const rawDurations = {
            ...
            formatTags: parseFloat(data.format?.tags?.DURATION || "0"),
            videoTags: parseFloat(videoStream?.tags?.DURATION || "0"),
          };
          ...
          // Only use raw stream when we have positive proof it will work in all browsers.
          const safeContainers = ["mp4", "mov", "m4v", "webm"];
          const safeAudio = ["aac", "mp3", "opus", "vorbis"];
          const safeVideo = ["h264", "vp8", "vp9", "av1"];

          const containerOk = safeContainers.some((c) => formatName.includes(c));
          const audioOk = safeAudio.includes(audioCodec);
          const videoOk = safeVideo.includes(videoCodec);
          const useDirectStream = containerOk && audioOk && videoOk;

          const reason = useDirectStream
            ? "native_ok"
            : !containerOk
          ...
          console.log(
            `Probe ${file.name}: container=${formatName} v=${videoCodec} a=${audioCodec} duration=${durationSec}s [raw: fmt=${rawDurations.format} v=${rawDurations.video} a=${rawDurations.audio} vtag=${mkvDurationVideo} atag=${mkvDurationAudio}] → ${useDirectStream ? "DIRECT" : "AAC"} (${reason})`
          );
          jsonResponse(res, 200, { useDirectStream, audioCodec, videoCodec, container: formatName, reason, durationSec });
        } catch {
          if (ffErr) console.log(`Probe ${file.name}: ffprobe error — ${ffErr.slice(0, 200)}`);
          console.log(`Probe ${file.name}: parse failed — routing to AAC transcode`);
          // Pessimistic default on any failure.
          jsonResponse(res, 200, { useDirectStream: false, reason: "probe_failed", durationSec: null });
        }
      });
      ff.on("error", (err) => {
        console.error(`Probe ${file.name}: spawn error — ${err.message}`);
        jsonResponse(res, 200, { useDirectStream: false, reason: "ffprobe_spawn_failed", durationSec: null });
      });
      return;
    }
    ...
    if (req.method === "GET" && path.startsWith("/stream-aac/")) {
      ...
      // Audio offset in milliseconds — positive = audio later, negative = audio earlier
      const audioOffsetMs = parseInt(url.searchParams.get("ao") || "0") || 0;
      const audioOffsetSec = audioOffsetMs / 1000;
      const file = findVideoFile(torrent, fileParam);
      if (!file) return jsonResponse(res, 404, { error: "No video file" });

      // Switch active selection to the requested episode if needed
      const prevAacFile = selectedVideoFile.get(infoHash);
      if (prevAacFile !== file) {
        if (prevAacFile) prevAacFile.deselect();
        file.select();
        selectedVideoFile.set(infoHash, file);
        console.log(`Episode selected (aac): ${file.name}`);
      }
      ...
      // If seeking into a partially downloaded file, wait for pieces at the target position
      // before handing the disk path to ffmpeg — otherwise ffmpeg reads zeroed bytes and crashes.
      if (seekSeconds > 0 && torrent.progress < 1) {
        const fileDur = parseFloat(url.searchParams.get("dur") || "0");
        if (fileDur > 0) {
          const estByte = Math.floor((seekSeconds / fileDur) * file.length);
          const pieceLen = torrent.pieceLength;
          const startPiece = Math.floor((file.offset + estByte) / pieceLen);
          const windowPieces = 10;
          const hasPieces = () => {
            for (let i = 0; i < windowPieces; i++) {
              const idx = startPiece + i;
              if (idx < torrent.pieces.length && !torrent.bitfield.get(idx)) return false;
            }
            return true;
          };

          if (!hasPieces()) {
            console.log(`[aac] Seek ${seekSeconds}s: waiting for pieces ${startPiece}–${startPiece + windowPieces - 1} to download...`);
            // Trigger WebTorrent to prioritize the needed range
            const priByte = Math.max(0, estByte - pieceLen);
            const priEnd = Math.min(file.length - 1, estByte + pieceLen * windowPieces);
            const priorityStream = file.createReadStream({ start: priByte, end: priEnd });
            priorityStream.on("data", () => {}); // drain to keep priority active
            priorityStream.on("error", () => {});

            let ready = false;
            for (let w = 0; w < 60; w++) {
              await new Promise((r) => setTimeout(r, 500));
              if (res.destroyed) { priorityStream.destroy(); return; }
              if (hasPieces()) { ready = true; break; }
            }
            priorityStream.destroy();
            if (!ready) {
              console.log(`[aac] Seek ${seekSeconds}s: timeout waiting for pieces — telling client to retry`);
              return jsonResponse(res, 202, { status: "buffering", message: "Downloading seek position..." });
            }
            console.log(`[aac] Seek ${seekSeconds}s: pieces ready, starting ffmpeg`);
          }
        }
      }

      if (audioOffsetMs !== 0) console.log(`[aac] Audio offset: ${audioOffsetMs}ms (${audioOffsetSec}s)`);

      // When audio offset is set, we use two inputs from the same file:
      //   input 0 = audio (shifted by itsoffset)
      //   input 1 = video (original timing)
      // This lets us shift audio forward or backward independently.
      const hasOffset = audioOffsetSec !== 0;
      const seekArgs = seekSeconds > 0 ? ["-ss", String(seekSeconds)] : [];
      // AAC encoder priming delay (~2048 samples ≈ 44ms at 48kHz) causes audio to start late
      // in fragmented MP4 (no edit list to compensate). Two-pronged fix:
      //   1. -itsoffset -0.044 : pre-shift the input 44ms earlier so the first real audio
      //      sample aligns with the first video frame after encoding.
      //   2. aresample=first_pts=0 : pins the first output audio sample to PTS=0, eliminating
      //      any residual gap from the priming silence.
      // When the user has set a manual offset, their -itsoffset value takes precedence and
      // the auto-compensation is skipped (they're explicitly controlling sync themselves).
      const autoOffsetArgs = hasOffset ? [] : ["-itsoffset", "-0.044"];
      const ffArgs = [
        "-loglevel", "warning",
        // Input 0: audio source
        ...(hasOffset ? ["-itsoffset", String(audioOffsetSec)] : autoOffsetArgs),
        ...seekArgs,
        "-i", diskPath,
        // Input 1: video source (original timing) — only when manual offset is active
        ...(hasOffset ? [...seekArgs, "-i", diskPath] : []),
        // Map: video from unshifted input, audio from (optionally shifted) input
        "-map", hasOffset ? "1:v:0" : "0:v:0",
        "-map", "0:a:0",
        "-c:v", "copy",
        "-c:a", "aac",
        "-b:a", "192k",
        "-ac", "2",
        // Pin first encoded audio sample to PTS=0 — eliminates residual priming silence gap
        "-af", "aresample=first_pts=0",
        // Prevent stalls on titles with heavy audio/video interleave imbalance
