...
// WebTorrent sorts selections by priority (torrent.js:_select), and playback's
// own read stream registers at priority 1. Selecting what you're watching above
// that, and a prefetch below it, means the piece picker starves the prefetch
// whenever the live stream needs bandwidth — no throttling of our own required.
const STREAM_PRIORITY = 10;
const PREFETCH_PRIORITY = 0;

const selectedVideoFile = new Map(); // infoHash -> currently selected file object
// infoHash -> Set of files being pulled ahead for "next episode".
//
// These are deliberately NOT in selectedVideoFile: that map is a single slot,
// and the switch latch in /stream and /probe deselects and purges whatever is
// in it. Since the browser re-enters that latch on every range request, a
// prefetched file parked there would be destroyed by the next buffer refill.
const prefetchedFiles = new Map();

function isPrefetched(infoHash, file) {
  return prefetchedFiles.get(infoHash)?.has(file) === true;
}

/**
 * A pull-ahead that is still pending — i.e. not the file already playing.
 * Once a prefetched episode becomes the one being watched it stops counting as
 * outstanding work, so the idle sweep can park the torrent normally again.
 */
function hasPendingPrefetch(infoHash) {
  const set = prefetchedFiles.get(infoHash);
  if (!set?.size) return false;
  const current = selectedVideoFile.get(infoHash);
  for (const f of set) if (f !== current) return true;
  return false;
}

function forgetPrefetch(infoHash, file) {
  const set = prefetchedFiles.get(infoHash);
  if (!set) return;
  set.delete(file);
  if (set.size === 0) prefetchedFiles.delete(infoHash);
}
    ...
    try {
      ...
      torrent.on("ready", () => {
        ...
        if (videoFiles.length === 1) {
          ...
          videoFiles[0].select(STREAM_PRIORITY);
...
function isBusy(infoHash) {
  ...
  // A prefetch has no playback session of its own, so the idle sweep would park
  // it — and pauseTorrent deselects every file, killing the pull-ahead outright.
  if (hasPendingPrefetch(infoHash)) return true;
...
function pauseTorrent(infoHash) {
  ...
  // Pausing deselected every file, so any pull-ahead is no longer running.
  prefetchedFiles.delete(infoHash);
...
function purgeFileBytes(torrent, file) {
  try {
    // Never reclaim something we deliberately pulled ahead.
    if (isPrefetched(torrent.infoHash, file)) {
      console.log(`Skipped purge (prefetched): ${file.name}`);
      return;
    }

    const pieceLen = torrent.pieceLength;
    let startPiece = Math.floor(file.offset / pieceLen);
    let endPiece = Math.floor((file.offset + file.length - 1) / pieceLen);

    // Piece boundaries don't line up with file boundaries, so the first and
    // last piece of this file usually carry bytes belonging to its neighbours.
    // If a neighbour is being prefetched, dropping the shared piece would strip
    // the very first bytes it needs — so leave the straddling pieces alone.
    const neighbours = prefetchedFiles.get(torrent.infoHash);
    if (neighbours?.size) {
      for (const other of neighbours) {
        const otherStart = Math.floor(other.offset / pieceLen);
        const otherEnd = Math.floor((other.offset + other.length - 1) / pieceLen);
        if (otherStart === endPiece) endPiece -= 1;
        if (otherEnd === startPiece) startPiece += 1;
      }
    }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST" && path.startsWith("/prefetch/")) {
      const infoHash = path.split("/")[2];
      const torrent = getTorrent(infoHash);
      if (!torrent) return jsonResponse(res, 404, { error: "Torrent not found" });

      const file = findVideoFile(torrent, url.searchParams.get("file"));
      if (!file) return jsonResponse(res, 404, { error: "No video file found" });

      // Already the episode being watched — nothing to pull ahead.
      if (selectedVideoFile.get(infoHash) === file) {
        return jsonResponse(res, 200, { prefetching: false, reason: "current" });
      }
      if (isPrefetched(infoHash, file)) {
        return jsonResponse(res, 200, { prefetching: true, reason: "already" });
      }

      // Deliberately does NOT touch selectedVideoFile: this file must survive
      // the switch latch that fires on every range request of what's playing.
      // Only ever one episode ahead. Replacing the target is what releases the
      // previous one for reclaiming once you've moved past it.
      file.select(PREFETCH_PRIORITY);
      prefetchedFiles.set(infoHash, new Set([file]));
      waitForHead(torrent, file).catch(() => {}); // make it instantly seekable
      console.log(`Prefetching next episode: ${file.name}`);

      return jsonResponse(res, 200, { prefetching: true, file: file.name });
    }
    ...
    if (req.method === "DELETE" && path.startsWith("/torrent/") && path.endsWith("/file")) {
      ...
      // Clear the slot too. Leaving it pointing at a file we just purged makes
      // the next /stream for that same file hit the "already selected" check,
      // skip select(), and download nothing at all.
      if (selectedVideoFile.get(infoHash) === file) selectedVideoFile.delete(infoHash);
      forgetPrefetch(infoHash, file);
