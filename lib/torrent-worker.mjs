...
function purgeFileBytes(torrent, file) {
  try {
    ...
    for (let i = startPiece; i <= endPiece; i++) {
      if (torrent.bitfield.get(i)) torrent.bitfield.set(i, false);
    }
  ...
  try {
    ...
    if (req.method === "GET" && path.startsWith("/probe/")) {
      ...
      if (!existsSync(diskPath) || !(await waitForHead(torrent, file))) {
        console.log(`Probe ${file.name}: head not ready — routing to AAC transcode (buffering)`);
        return jsonResponse(res, 200, { useDirectStream: false, reason: "head_not_ready", durationSec: null });
      }
    ...
    if (req.method === "GET" && path.startsWith("/stream-aac/")) {
      ...
      if (prevAacFile !== file) {
        if (prevAacFile) { prevAacFile.deselect(); purgeFileBytes(torrent, prevAacFile); }
        file.select();
        selectedVideoFile.set(infoHash, file);
        waitForHead(torrent, file).catch(() => {}); // prime head pieces early
      ...
      const diskPath = join(CACHE_PATH, file.path);
      if (!existsSync(diskPath) || !(await waitForHead(torrent, file))) {
        console.log(`[aac] ${file.name}: head not ready — telling client to retry`);
        return jsonResponse(res, 202, { status: "buffering", message: "Downloading file header..." });
      }
      ...
      // Dual-input offset mode requires two seekable -i diskPath handles — impossible on a pipe
      // and unsafe on sparse files (unallocated regions read as zeros → ffmpeg crash).
      if (hasOffset && torrent.progress < 1) {
        console.log(`[aac] Audio offset requested but file is ${(torrent.progress * 100).toFixed(1)}% — needs full download`);
        return jsonResponse(res, 409, {
          error: "audio_offset_needs_complete_file",
          message: "Wait for the episode to finish downloading before adjusting audio offset.",
        });
      }
      ...
      // in fragmented MP4. Pre-shift input 44ms + pin first audio PTS=0 to compensate.
      // Skipped when the user sets a manual offset (they control sync explicitly).
      const autoOffsetArgs = hasOffset ? [] : ["-itsoffset", "-0.044"];

      let ffArgs;
      if (hasOffset) {
        // Fully-downloaded file — safe to use two seekable disk-path inputs for offset mode.
        ffArgs = [
          "-loglevel", "warning",
          "-itsoffset", String(audioOffsetSec),
          ...seekArgs,
          "-i", diskPath,
          ...seekArgs, "-i", diskPath,
          "-map", "1:v:0",
        ...
        // Pipe WebTorrent's stream into ffmpeg stdin so it never reads sparse zeros.
        // pipe:0 is non-seekable at the OS level — ffmpeg cannot lseek on it regardless.
        // -f <container> is required because pipe:0 has no extension for auto-detection.
        // -ss AFTER -i performs output/demux-skip seek, compatible with non-seekable input.
        const ext = file.name.split(".").pop().toLowerCase();
        const inputFormat = { mkv: "matroska", webm: "matroska", mp4: "mp4", m4v: "mp4", ts: "mpegts", avi: "avi" }[ext] || "matroska";
        ffArgs = [
          "-loglevel", "warning",
          "-f", inputFormat,
          ...autoOffsetArgs,
      ...
      if (!hasOffset) {
        const torrentStream = file.createReadStream({ start: 0 });
        torrentStream.pipe(ff.stdin);
        torrentStream.on("error", (err) => {
          console.error("[aac] torrent stream error:", err.message);
          try { ff.kill("SIGKILL"); } catch {}
        });
        ff.stdin.on("error", () => {}); // ignore EPIPE when ffmpeg exits early
        req.on("close", () => {
          try { torrentStream.destroy(); } catch {}
          try { ff.kill("SIGKILL"); } catch {}
        });
      } else {
